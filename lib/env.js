...
// In local mode (GRIMOIRE_ROOT set), falls back to the repo's canonical config
// when the user-space cache is absent — the server owns the authority and must
// never be unresolvable because a cache was cleared.
function _lbl() {
  try { return JSON.parse(fs.readFileSync(_cachePath(), 'utf8')) }
  catch {
    // Local-mode fallback: read from repo config when cache is absent
    if (process.env.GRIMOIRE_ROOT) {
      try {
        const repoCfg = JSON.parse(fs.readFileSync(_repoConfigPath(), 'utf8'))
        if (repoCfg.endpoints) return repoCfg
      } catch { /* repo config absent — return empty */ }
    }
    return {}
  }
}
...
module.exports = { config, isLocal, isRemote, requireMode, lblEndpoint, refreshLblCache, clearLblCache, lblCacheMeta, resolveGoogleCseKeys, _cachePath, _metaPath, _repoConfigPath }
