...
const config = {
  ...
  // Grimoire server address (remote mode) — e.g. http://grimoire.local:3663
  host:     process.env.GRIMOIRE_HOST   || null,

  // Ollama base URL
  ollama:   process.env.OLLAMA_HOST     || 'http://grimoire.local:11434',
...
function requireMode(required = 'any') {
  if (required === 'local' && !isLocal) {
    ...
    console.error('Run it on grimoire.local, or set GRIMOIRE_ROOT in .env.')
    process.exit(1)
  }
  if (required === 'remote' && !isRemote) {
    console.error('This command requires a running Grimoire server (GRIMOIRE_HOST).')
    console.error('Set GRIMOIRE_HOST=http://grimoire.local:3663 in .env, then run: grim serve')
    process.exit(1)
  }
