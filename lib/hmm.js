...
// Every status the state machine can emit — viewers must map all of these.
const STATUSES = ['working', 'conversing', 'waiting-on-user', 'waiting', 'sleeping', 'retired']
 ...
 * @param {object} threadData — { project, thread, roadmapOpen }
 * @param {number} now — epoch seconds
 * @returns {object} — { project, roadmapOpen, retired, participants, livePhase,
 *   latestState, lastActivitySec, conversingPair }
 */
function projectStatus(threadData, now) {
  ...
  return {
    ...
    retired,
    participants,
    livePhase: latest ? latest.phase : null,
    latestState: latest ? latest.state : null,
    lastActivitySec: _latestAge(thread, now),
    conversingPair: _conversingPair(thread, ACTIVE_SEC, now),
  }
}
...
/**
 * Roles of the two most recent alternating senders when the thread is
 * actively conversing — null when not conversing. Drives the viewer's
 * "turn toward each other" animation.
 */
function _conversingPair(thread, activeSec, now) {
  if (!_isConversing(thread, activeSec)) return null
  const latest = thread[thread.length - 1]
  if (now - _tsEpoch(latest) > activeSec) return null
  const prev = thread[thread.length - 2]
  if (!prev) return null
  return [latest.from, prev.from]
}
...
module.exports = { scanProjects, projectStatus, toPrometheus, ACTIVE_SEC, IDLE_SEC, STATUSES }
