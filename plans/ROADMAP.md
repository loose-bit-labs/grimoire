...
Binding for all phases. Last updated 2026-09-02 (off-phase tooling: shipped **grim portal** — Claude Code launcher with an in-process filter proxy that strips `image` blocks on the wire so a text-only local model (Qwen3.8-Flash) never chokes; `bin/grim-portal.js` (proxy :13431), positional `<host>` (direct, no lbl-config lookup), pluggable filters (strip default; opt-in `caption` → local llava), Option B repo-self-hosts-as-marketplace, `--help` shows portal params before claude's, `CLAUDE_CODE_MAX_CONTEXT_TOKENS` fix; 20 tests; `87038b0`/`a923349`/`c874c6d`; KB `system_grim_portal_*`. Promoted `~/tmp/hmm-ww.sh` → **grim hmm --watch** — terminal Guild Hall doorbell (`bin/grim-hmm.js`; portrait-of-who's-up via timg, spotlight of latest exchange; `latestAcross` epoch-sort fix so a missing-ts message can't hijack the spotlight); `97c2e03`. Pact idle — phase 86 closed.) 2026-09-01 (phase 86 re-report #0418 (minion on aid) — both #0417 fixes implemented + re-verified: fix 1 awaits at `bin/grim-server.js:349/:374` + repo-wide sweep clean; fix 2 atomic in-place regen (inode-preserving) + reload-with-verify and verified-restart fallback (Docker file-mount pins the original inode — a rename is invisible to the running container); success checks (a) mid-crash no-op and (b) live-config reflection sandbox-proven with pasted output; incident disclosed: two accidental prod `grim-prometheus` restarts during pre-seam sandbox runs (healthy after, config never written by minion); other session's telemetry cutover landed 02:00 — on-disk config byte-identical to the generator's, prod serving 9 targets. **ACCEPTED #0419 (2026-09-01)** — mage independently re-verified (own crash-drill re-run of check (a), own repo sweep, live 9-target probe, sha256-identical regen, suite 529/0/1); shipped `26cc478` (8 files), review archived `626b224`; mage one-line help-text fix at `bin/grim-rig.js:1697` (stale `aid:9090` default → `localhost:9090` per the code — cleared the bare-hostname pre-commit hook).) 2026-08-31: phase 86 report #0416 (minion on aid) → **REVISE #0417** — roster derivation verified live and standing (9 boxes, overlay badges, client mode, rig.json 9→2); fix 1: un-awaited `loadBoxesGraceful()` consumers at `bin/grim-server.js:349/:374` (async flip regression); fix 2: plan amendment `6e3efdd` (post-report) adds atomic-regen + inode-aware-reload requirements after the GPU-panel 0-byte-config incident. Phase 89 **accepted #0414** — shipped `f4cea4e`, review archived `bf2de58`; 86 briefed to minion (after 89). 2026-08-30: phase 87 **accepted #0411** — shipped `a74e933`, review archived `3f43df1`; 89 briefed to minion, pulled ahead of 86. 2026-08-29: Track G-v3: 82/83/84 accepted+shipped — `687aa9e`/`7485879`/`e5494ca`+`1e6dc52`; 85 drain complete 10/10 real qwen3.8 digests via meinherz:11311, report #0407 → **accepted #0408**, shipped `1546dd5`, review archived `0a245bf`; 87 accepted #0411 (2026-08-30), shipped `a74e933`, archived `3f43df1`; 89 briefed (pulled ahead of 86). Briefed this session: 86 fleet-roster-derive, 87 config-read robustness, 88 `/hunter` loop skill, 89 dig-clone hardening. Bounty track re-scoped to piecemeal-MVP **74+76+88** (75/77 deferred); first hunter = a dedicated competent-model session. New rulings: sources-of-truth, log-timestamping. Seed bounties filed: safe-AI-workspace, swandive-thread-per-topic. **74 is queued, not accepted** — earlier header typo. Pact running on aid (tbona offline; sessions repointed off tbona 2026-08-29/30).)
...
| Phase | Brief | What lands | Status |
|-------|-------|-----------|--------|
| 86 | plans/phase-86.md | **derive the fleet roster from the host registry** — `lib/fleet.js` `loadFleet()` merges registry hosts (`hardware/inventory`, local or `GET /api/hosts/inventory`) with `rig.json` reinterpreted as a keyed **service-check overlay**; repoint `grim rig` `loadBoxes()` + the telemetry generators (`generate-dashboard.js`/`generate-scrape.sh`) at it; a newly-registered box auto-appears with no hand-edit. | **ACCEPTED (#0419, 2026-09-01) — shipped `26cc478`, review archived `626b224`** — both #0417 fixes independently re-verified by the mage (crash-drill (a) reproduced: mid-run error leaves the live file byte-identical, same inode; own flipped-export sweep clean; live prod serving the 9-target set; fresh regen sha256-identical to the cutover file; suite 529/0/1); fix 1 = awaits at `bin/grim-server.js:349/:374`; fix 2 (amendment `6e3efdd`) = atomic in-place regen (same inode — the file mount pins it) + reload-with-verify, verified-restart fallback when the mount inode diverged; latent fixes: `FLEET_JS` default path + `|| fail` generator gating; disclosed incident (2 pre-seam prod prometheus restarts, verified intact, config never written by minion); mage doc fix at `bin/grim-rig.js:1697` (stale prometheus default) — Track F cont.; independent of Track G-v3 |
