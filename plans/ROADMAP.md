...
| 85 | plans/phase-85.md | **swandive as transport** (fLimfLaMs) — submit-and-return to the queue, `onReady` catch-up scans DM history for un-answered drops, embed posted on worker completion; **backfill the 11 recorded URLs** as the first enqueue. | in progress (briefed 2026-08-26, #0400) — implementation + 19/19 tests written 2026-08-28 (SwandiveDiscordBot transport + scripts/backfill-dive-queue.js); drain OOM-blocked (decayed lbl-config cache hid `use.coding` → `resolveModel` recursion; repaired via `grim config sync`, hierophant #0403 — guard fix scoped to 87) — **drain complete 2026-08-29: 10/10 researched, real digests via meinherz:11311**; awaiting minion report → verdict; swandive.service cutover pending post-acceptance — Track G-v3; depends 84 (satisfied) |
